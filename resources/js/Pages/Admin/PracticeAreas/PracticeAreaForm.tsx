...
export default function PracticeAreaForm({ data, setData, errors, processing, submitLabel, cancelHref, currentImage }: Props) {
        ...
        <div className="grid lg:grid-cols-3 gap-6">
            <div className="lg:col-span-2 space-y-5">
                <div className="bg-white rounded-sm shadow-sm p-6 space-y-5">
                    ...
                    <div>
                        <label className="block text-sm font-medium text-gray-700 mb-1.5">Card Summary</label>
                        <textarea rows={3} value={data.summary} onChange={e => setData('summary', e.target.value)} className="w-full border border-gray-300 rounded-sm px-3.5 py-2.5 text-sm focus:outline-none focus:ring-2 focus:ring-gold-500 resize-none" />
                    </div>

                    <div>
                        <label className="block text-sm font-medium text-gray-700 mb-1.5">Main Overview Paragraph</label>
                ...
                <div className="bg-white rounded-sm shadow-sm p-6 space-y-5">
                    <div>
                        ...
                        <textarea rows={6} value={data.services} onChange={e => setData('services', e.target.value)} className="w-full border border-gray-300 rounded-sm px-3.5 py-2.5 text-sm focus:outline-none focus:ring-2 focus:ring-gold-500 resize-none" />
                    </div>
                    <div>
                        <label className="block text-sm font-medium text-gray-700 mb-1.5">Client Scenarios <span className="text-gray-400 font-normal">(Scenario | Answer, one per line)</span></label>
                        <textarea rows={5} value={data.client_needs} onChange={e => setData('client_needs', e.target.value)} className="w-full border border-gray-300 rounded-sm px-3.5 py-2.5 text-sm focus:outline-none focus:ring-2 focus:ring-gold-500 resize-none" />
                    </div>
                    <div>
                        <label className="block text-sm font-medium text-gray-700 mb-1.5">Why Choose Trill Paragraph</label>
                        <textarea rows={4} value={data.why_trill} onChange={e => setData('why_trill', e.target.value)} className="w-full border border-gray-300 rounded-sm px-3.5 py-2.5 text-sm focus:outline-none focus:ring-2 focus:ring-gold-500 resize-none" />
                    </div>
                    <div>
                        <label className="block text-sm font-medium text-gray-700 mb-1.5">Related Areas <span className="text-gray-400 font-normal">(Title | /practice-areas/slug)</span></label>
                        <textarea rows={4} value={data.related_areas} onChange={e => setData('related_areas', e.target.value)} className="w-full border border-gray-300 rounded-sm px-3.5 py-2.5 text-sm focus:outline-none focus:ring-2 focus:ring-gold-500 resize-none" />
                    </div>
                </div>
            </div>
            ...
            <div className="space-y-5">
                <div className="bg-white rounded-sm shadow-sm p-5">
                    ...
                    <label className="flex items-center gap-2.5 cursor-pointer mb-3"><input type="checkbox" checked={data.is_active} onChange={e => setData('is_active', e.target.checked)} className="w-4 h-4 accent-gold-500" /><span className="text-sm text-gray-700">Show on website</span></label>
                    <label className="flex items-center gap-2.5 cursor-pointer mb-5"><input type="checkbox" checked={data.show_in_nav} onChange={e => setData('show_in_nav', e.target.checked)} className="w-4 h-4 accent-gold-500" /><span className="text-sm text-gray-700">Show in navigation</span></label>
                ...
                <div className="bg-white rounded-sm shadow-sm p-5 space-y-4">
                    <h3 className="font-medium text-navy-950 text-sm">Practice Area Photo</h3>
                    ...
                    <div onClick={() => fileRef.current?.click()} className="border-2 border-dashed border-gray-300 hover:border-gold-400 rounded-sm p-4 text-center cursor-pointer transition-colors">
                        <p className="text-xs text-gray-500">{data.image_file ? data.image_file.name : 'Click to upload photo'}</p>
                    </div>
